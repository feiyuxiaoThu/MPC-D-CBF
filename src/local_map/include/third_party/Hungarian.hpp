...
HungarianAlgorithm::HungarianAlgorithm() : iteration_count(0)
{
}
							   ...
							   int minDim)
{
  ...
  if (iteration_count > MAX_ITERATIONS) {
    cerr << "Hungarian algorithm: Maximum iterations exceeded, forcing termination" << endl;
    // 强制构建当前最佳分配
    buildassignmentvector(assignment, starMatrix, nOfRows, nOfColumns);
    return;
  }
	...
	if (!coveredRows[row])
	  for (col = 0; col < nOfColumns; col++)
		if (!coveredColumns[col])
		{
		  ...
		  // 添加数值稳定性检查
		  if (value == value && value >= 0 && value < h && value < DBL_MAX)  // 检查NaN和有效值
			h = value;
		}

  // 如果没有找到未覆盖元素或h无效，终止算法
  if (!found_uncovered || h == DBL_MAX || h != h) {  // h != h 检查NaN
    cerr << "Hungarian algorithm: No valid uncovered elements found, terminating" << endl;
    buildassignmentvector(assignment, starMatrix, nOfRows, nOfColumns);
    return;
  }
  ...
  for (row = 0; row < nOfRows; row++)
	if (coveredRows[row])
	  for (col = 0; col < nOfColumns; col++) {
	    double new_value = distMatrix[row + nOfRows * col] + h;
	    // 检查溢出
	    if (new_value == new_value && new_value < DBL_MAX)  // 检查NaN和溢出
	      distMatrix[row + nOfRows * col] = new_value;
	  }
  ...
  for (col = 0; col < nOfColumns; col++)
	if (!coveredColumns[col])
	  for (row = 0; row < nOfRows; row++) {
	    double new_value = distMatrix[row + nOfRows * col] - h;
	    // 检查下溢和负值
	    if (new_value == new_value && new_value >= 0)  // 检查NaN和负值
	      distMatrix[row + nOfRows * col] = new_value;
	    else
	      distMatrix[row + nOfRows * col] = 0.0;  // 设为最小值
	  }
